...
const MAX_BOOK_MOVES = 30; // 15 Full moves
...
const KID_BOOK: BookLine[] = [
    ...
    { name: 'KID (Beg): vs Fianchetto', moves: ['d4', 'Nf6', 'c4', 'g6', 'g3', 'Bg7', 'Bg2', 'O-O', 'Nf3', 'd6', 'O-O', 'Nbd7', 'Nc3', 'e5', 'e4', 'c6', 'Re1', 'exd4', 'Nxd4', 'Nc5'] },
...
export const getBookMove = (
    ...
    // - Stop using book moves after Move 15 (30 half-moves)
    if (moveCount >= MAX_BOOK_MOVES) return null;
    
    // - If in Check, let engine handle it (Tactics required)
    if (game.inCheck()) return null;

    // 1. Try Specific Book Lines (STRICT MATCHING)
    ...
    if (verboseHistory.length > 0) {
        ...
        // Heuristic: If last move was a capture, tactic likely.
    ...
    if (genericSequence.length > 0) {
        ...
        // Use a temp game to simulate and check safety
        ...
        for (const target of genericSequence) {
            ...
            if (target.san === 'O-O') {
                // If we can't castle and haven't castled, maybe skipping this step is fine if it's illegal.
            ...
            // B: Candidate move determination
            let candidateSan = "";
            if (target.san) {
                if (legalSanMoves.includes(target.san)) candidateSan = target.san;
            } else if (target.to && target.piece) {
                const legalMoves = game.moves({ verbose: true });
                const move = legalMoves.find(m => m.to === target.to && m.piece === target.piece);
                if (move) candidateSan = move.san;
            }

            // C: Safety Check (Blunder Detection)
            if (candidateSan) {
                try {
                    // 1. Reset temp game for clean simulation
                    tempGame.load(game.fen());
                    const moveResult = tempGame.move(candidateSan);
                    
                    if (moveResult) {
                        // 2. Is the destination square attacked by opponent?
                        // If we are placing a piece on an attacked square, we assume it's a BLUNDER 
                        // in the context of a "blind" generic system.
                        // (Specific books might do this for gambits, but generic system should be safe).
                        const toSquare = moveResult.to;
                        
                        // @ts-ignore: chess.js v1.x supports isAttacked
                        if (typeof tempGame.isAttacked === 'function') {
                             // @ts-ignore
                             if (tempGame.isAttacked(toSquare, opponentColor)) {
                                 // Unsafe generic move -> let engine decide
                                 return null;
                             }
                        }

                        // 3. Did we just get checkmated? (Extreme edge case)
                        if (tempGame.isCheckmate()) return { san: candidateSan, name: systemName + " (Mate)" };

                        // If safe, return it
                        return { san: candidateSan, name: systemName };
                    }
                } catch (e) {
                    // Logic error in simulation? Skip.
                    continue;
                }
            }
        }
